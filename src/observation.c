...
static size_t flatten_pokemon(float* out, size_t idx, const ObsPokemon* p) {
    ...
    idx = write_flag(out, idx, p->torment_active);
    idx = write_scalar(out, idx, p->torment_turns / 5.0f);
    idx = write_flag(out, idx, p->heal_block_active);
    idx = write_scalar(out, idx, p->heal_block_turns / 5.0f);
    idx = write_flag(out, idx, p->embargo_active);
    ...
    idx = write_flag(out, idx, p->flinch_active);
    ...
    idx = write_flag(out, idx, p->seed_active);
    ...
    idx = write_flag(out, idx, p->charge_active);
    idx = write_scalar(out, idx, p->charge_turns / 5.0f);
