...
typedef struct {
    ...
    double awr_weight_sum;
    double awr_weight_square_sum;
    float awr_weight_min;
    float awr_weight_max;
...
typedef enum {
    TRAIN_METHOD_SUPERVISED = 0,
    TRAIN_METHOD_POLICY_GRADIENT = 1,
    TRAIN_METHOD_PPO = 2,
    TRAIN_METHOD_ADVANTAGE_WEIGHTED_IMITATION = 3
} TrainMethod;
...
static void rl_training_summary_record_trainer(RlTrainingSummary* summary, const GruTrainer* trainer) {
    int first_awr_weights;
    if (!summary || !trainer || trainer->last_rl_labels == 0) {
        return;
    }
    first_awr_weights = summary->label_weight_sum == 0;
    ...
    summary->awr_weight_sum += trainer->last_awr_weight_sum;
    summary->awr_weight_square_sum += trainer->last_awr_weight_square_sum;
    if (first_awr_weights || trainer->last_awr_weight_min < summary->awr_weight_min) {
        summary->awr_weight_min = trainer->last_awr_weight_min;
    }
    if (first_awr_weights || trainer->last_awr_weight_max > summary->awr_weight_max) {
        summary->awr_weight_max = trainer->last_awr_weight_max;
    }
...
static int write_rl_training_summary_json(
    ...
    fputs("  \"training_method\": ", out);
    json_write_escaped(out, summary->training_method ? summary->training_method : "");
    fputs(",\n", out);
    ...
    fprintf(out, "  \"awr_temperature\": %.9g,\n", trainer->awr_temperature);
    fprintf(out, "  \"awr_max_weight\": %.9g,\n", trainer->awr_max_weight);
    fprintf(out, "  \"critic_updated\": %s,\n",
        summary->training_method && strcmp(summary->training_method, "advantage_weighted_imitation") == 0
            ? "false" : "true");
    ...
    fprintf(out, "  \"awr_weight_mean\": %.6f,\n", summary->label_weight_sum > 0 ? summary->awr_weight_sum / (double)summary->label_weight_sum : 0.0);
    fprintf(out, "  \"awr_weight_min\": %.6f,\n", summary->awr_weight_min);
    fprintf(out, "  \"awr_weight_max_observed\": %.6f,\n", summary->awr_weight_max);
    fprintf(out, "  \"awr_effective_labels\": %.6f,\n",
        summary->awr_weight_square_sum > 0.0
            ? summary->awr_weight_sum * summary->awr_weight_sum / summary->awr_weight_square_sum
            : 0.0);
...
static int train_from_input_file(
    ...
    int rl_mode = method != TRAIN_METHOD_SUPERVISED;
    int ppo_mode = method == TRAIN_METHOD_PPO;
    int awr_mode = method == TRAIN_METHOD_ADVANTAGE_WEIGHTED_IMITATION;
    int guarded_policy_mode = ppo_mode || awr_mode;
    ...
    rl_summary.training_method = method == TRAIN_METHOD_ADVANTAGE_WEIGHTED_IMITATION
        ? "advantage_weighted_imitation"
        : (method == TRAIN_METHOD_PPO
            ? "ppo"
            : (method == TRAIN_METHOD_POLICY_GRADIENT ? "policy_gradient" : "supervised"));
    if (rl_mode) {
    ...
    if (output_checkpoint_loaded && guarded_policy_mode && input_kind == TRAIN_INPUT_EPISODE_BATCH) {
        char* resolved_parent_path;
        if (!parent_checkpoint_metadata || !*parent_checkpoint_metadata) {
            fprintf(stderr,
                "[train-live-policy] --parent-checkpoint is required to verify behavior-policy initialization\n");
            gru_model_destroy(model);
            free(resolved_checkpoint_path);
            return 1;
        }
        ...
        if (!resolved_parent_path ||
            ...
            fprintf(stderr,
                "[train-live-policy] output checkpoint does not match --parent-checkpoint; refusing off-policy initialization\n");
            free(resolved_parent_path);
            gru_model_destroy(model);
            free(resolved_checkpoint_path);
            return 1;
    ...
    if (rl_mode && anchor_checkpoint_path && *anchor_checkpoint_path && anchor_kl_coef > 0.0f) {
        ...
        if (guarded_policy_mode && input_kind == TRAIN_INPUT_EPISODE_BATCH &&
    ...
    if (rl_mode) {
        ...
        trainer.awr_temperature = awr_temperature;
        trainer.awr_max_weight = awr_max_weight;
        ...
        if (guarded_policy_mode) {
            printf("[train-%s] target_kl_source=%s\n",
                awr_mode ? "awr" : "ppo",
    ...
    if (rl_mode) {
        ...
        rl_summary.minibatch_episodes = policy_minibatch_episodes > 0 ? policy_minibatch_episodes : 1;
    ...
    for (epoch = 1; epoch <= epochs; ++epoch) {
        ...
        for (size_t order_i = 0; order_i < train_sessions;) {
            ...
            if (rl_mode) {
                if (guarded_policy_mode) {
                    size_t configured_batch = (size_t)(policy_minibatch_episodes > 0 ? policy_minibatch_episodes : 1);
                    ...
                    if (!minibatch) {
                        fprintf(stderr, "Failed to allocate policy minibatch\n");
                        free(train_indices);
                        free(val_indices);
                        env_runtime_free(&runtime);
                        gru_model_destroy(model);
                    ...
                    if (!(awr_mode
                            ? gru_trainer_advantage_weighted_minibatch(
                                &trainer, model, minibatch, episodes_this_update)
                            : gru_trainer_ppo_minibatch(
                                &trainer, model, minibatch, episodes_this_update))) {
                        free(minibatch);
                        fprintf(stderr, "Failed %s minibatch update\n", awr_mode ? "AWR" : "PPO");
                        free(train_indices);
                        free(val_indices);
                        env_runtime_free(&runtime);
                        gru_model_destroy(model);
                ...
                if (guarded_policy_mode && trainer.target_kl > 0.0f) {
            ...
            order_i += episodes_this_update;
            {
                ...
                if (rl_mode) {
                    ...
                    if (awr_mode) {
                        double effective_labels = trainer.last_awr_weight_square_sum > 0.0
                            ? trainer.last_awr_weight_sum * trainer.last_awr_weight_sum /
                                trainer.last_awr_weight_square_sum
                            : 0.0;
                        printf("[train-awr] weight_mean=%.4f weight_min=%.4f weight_max=%.4f effective_labels=%.1f critic_updated=0\n",
                            trainer.last_rl_labels > 0
                                ? trainer.last_awr_weight_sum / (double)trainer.last_rl_labels
                                : 0.0,
                            trainer.last_awr_weight_min,
                            trainer.last_awr_weight_max,
                            effective_labels);
                    }
                } else {
            ...
            if (policy_early_stop) {
                break;
            }
        }

        if (policy_early_stop) {
            printf("[train-%s] early stop after epoch=%d episodes=%zu/%zu reason=%s kl_trigger=%.4f target_kl=%.4f kl_source=%s\n",
                awr_mode ? "awr" : "ppo",
                epoch,
                trained_in_epoch,
                train_sessions,
    ...
    if (rl_mode && rl_summary.target_kl_hard_stop) {
        ...
        fprintf(stderr,
            "[train-%s] rejected update; checkpoint was not published after hard KL stop\n",
            awr_mode ? "awr" : "ppo");
        free(train_indices);
        free(val_indices);
        env_runtime_free(&runtime);
...
static int run_counterfactual_q_fit_check(
    ...
    if (external_holdout && !counterfactual_dataset_load(
        ...
        fprintf(stderr,
            "[counterfactual-q] failed to load holdout batch '%s'; verify its policy tag and pair integrity\n",
            holdout_batch_path);
        goto cleanup;
    }
    ...
    if (overfit_pair_count > 0) {
        if (!counterfactual_dataset_overfit_subset(
                &dataset, overfit_pair_count, &split)) {
            fprintf(stderr,
...
static int run_counterfactual_policy_preference_check(
    ...
    if (!batch_path || !checkpoint_path || !report_path || epochs == 0 ||
            ...
            preferred_action_nll_coefficient < 0.0f ||
    ...
    external_holdout = overfit_pair_count == 0 && (training_source_is_manifest ||
        (holdout_batch_path && *holdout_batch_path));
    if (external_holdout &&
    ...
    if (overfit_pair_count > 0) {
        if (!counterfactual_dataset_overfit_subset(
                &dataset, overfit_pair_count, &split)) {
            fprintf(stderr,
                "[counterfactual-policy] could not build the real-data overfit subset\n");
            goto cleanup;
        }
    } else if (!counterfactual_dataset_split(
            &dataset, external_holdout ? &holdout_dataset : NULL,
    ...
    if (!counterfactual_dataset_split_filter_confidence(
            &split, minimum_pair_confidence)) {
        fprintf(stderr,
            "[counterfactual-policy] confidence filter left an empty or invalid split\n");
        goto cleanup;
    }
    if (overfit_pair_count > 0) overfit_pair_count = split.train_count / 2u;
    printf("[counterfactual-policy] mode=%s sources=%zu train=%zu selection=%zu holdout=%zu pairs holdout_source=%s epochs=%zu minibatch_pairs=%zu learning_rate=%.9g beta=%.6g preferred_nll_coef=%.6g anchor_kl=%.6g max_mean_kl=%.6g min_confidence=%.6g\n",
        overfit_pair_count > 0 ? "real_data_overfit" : "generalization",
        ...
        preferred_action_nll_coefficient,
        anchor_kl_coefficient, max_mean_policy_kl, minimum_pair_confidence);
    if (!learning_diagnostic_run_counterfactual_policy_preference_fit(
            ...
            split.holdout, split.holdout_count,
            epochs, minibatch_pairs, early_stop_patience, shuffle_seed,
            learning_rate, preference_beta, preferred_action_nll_coefficient,
    ...
    if (overfit_pair_count > 0) {
        /* Reusing the training pairs can validate optimization, not generalization. */
        result.policy_signal_detected = 0;
    } else {
        result.counterfactual_overfit_passed = 0;
    }
    publication_requested =
        output_checkpoint_path && *output_checkpoint_path;
    ...
    if (!learning_diagnostic_write_counterfactual_policy_preference_report(
            ...
            preference_beta, preferred_action_nll_coefficient,
    ...
    printf("[counterfactual-policy] signal=%d overfit_passed=%d loss=%.6f before_loss=%.6f direction=%.4f preferred_up=%.4f preferred_log_delta=%.6f ranking=%.4f mean_kl=%.6f max_kl=%.6f published=%d report=%s\n",
        ...
        result.after_holdout.confidence_weighted_preferred_probability_increase_rate,
        result.after_holdout.confidence_weighted_mean_preferred_log_probability_delta,
        result.after_holdout.confidence_weighted_pair_ranking_accuracy,
...
static int showdown_client_main(int argc, char** argv) {
    ...
    int counterfactual_policy_overfit_command = argc >= 2 &&
        strcmp(argv[1], "--check-counterfactual-policy-preference-overfit") == 0;
    int counterfactual_policy_command = argc >= 2 &&
        (strcmp(argv[1], "--check-counterfactual-policy-preference") == 0 ||
         ...
         counterfactual_policy_overfit_command);
    ...
    float policy_preference_preferred_nll_coef;
    float policy_preference_anchor_kl;
    float policy_preference_max_mean_kl;
    float policy_preference_min_confidence;
    ...
    int awr_command = argc >= 2 && strcmp(argv[1], "--train-live-awr") == 0;
    ...
    rl_entropy_coef = parse_float_flag(argc, argv, "--entropy-coef",
        (ppo_command || awr_command) ? rl_defaults.ppo_entropy_coef : rl_defaults.policy_gradient_entropy_coef);
    ...
    policy_preference_epochs = parse_int_flag(
        argc, argv, "--epochs",
        counterfactual_policy_overfit_command ? 200 : 100);
    ...
    policy_preference_preferred_nll_coef = parse_float_flag(
        argc, argv, "--preference-winner-nll-coef",
        counterfactual_policy_overfit_command ? 1.0f : 0.25f);
    policy_preference_anchor_kl = parse_float_flag(
        argc, argv, "--preference-anchor-kl-coef",
        counterfactual_policy_overfit_command ? 0.0f : 0.01f);
    policy_preference_max_mean_kl = parse_float_flag(
        argc, argv, "--preference-max-mean-kl",
        counterfactual_policy_overfit_command ? 10.0f : 0.02f);
    policy_preference_min_confidence = parse_float_flag(
        argc, argv, "--preference-min-confidence", 0.0f);
    ...
    if ((counterfactual_q_overfit_command ||
            counterfactual_policy_overfit_command) &&
            counterfactual_overfit_pairs <= 0) {
        fprintf(stderr,
            "counterfactual overfit diagnostics require --counterfactual-overfit-pairs > 0\n");
        return 1;
    }
    if (counterfactual_policy_overfit_command &&
            ((policy_preference_output_path && *policy_preference_output_path) ||
             (counterfactual_holdout_batch_path &&
                *counterfactual_holdout_batch_path))) {
        fprintf(stderr,
            "policy-preference overfit cannot publish a checkpoint or use a holdout batch\n");
        return 1;
    }
    if (counterfactual_policy_command &&
            (policy_preference_epochs <= 0 || policy_preference_seed < 0 ||
             ...
             policy_preference_preferred_nll_coef < 0.0f ||
             policy_preference_anchor_kl < 0.0f ||
             !(policy_preference_max_mean_kl > 0.0f) ||
             policy_preference_min_confidence < 0.0f ||
             policy_preference_min_confidence > 1.0f)) {
        fprintf(stderr,
            "counterfactual policy preferences require positive epochs, minibatch size, patience, beta, and max mean KL; confidence must be between zero and one\n");
        return 1;
    }
    ...
    if (awr_command &&
            (awr_minibatch_episodes <= 0 || awr_temperature <= 0.0f || awr_max_weight < 1.0f)) {
        fprintf(stderr,
            "--train-live-awr requires --awr-minibatch-episodes > 0, --awr-temperature > 0, and --awr-max-weight >= 1\n");
        return 1;
    }
    ...
    if (argc >= 2 &&
            ...
            strcmp(argv[1], "--train-live-awr") == 0 ||
    ...
    if (argc >= 6 && counterfactual_policy_manifest_command) {
        return run_counterfactual_policy_preference_check(
            ...
            policy_preference_output_path, 0u,
            ...
            policy_preference_beta, policy_preference_preferred_nll_coef,
            policy_preference_anchor_kl,
            policy_preference_max_mean_kl,
            policy_preference_min_confidence,
            (unsigned int)validation_seed,
            (unsigned int)policy_preference_seed,
            adam_beta1, adam_beta2, adam_epsilon);
    }
    if (argc >= 5 && counterfactual_policy_overfit_command) {
        return run_counterfactual_policy_preference_check(
            argv[2], "", 0, argv[3], argv[4], "",
            (size_t)counterfactual_overfit_pairs,
            (size_t)policy_preference_epochs,
            (size_t)policy_preference_minibatch_pairs,
            (size_t)policy_preference_early_stop_patience,
            learning_rate_override > 0.0f ? learning_rate_override : 0.001f,
    ...
    if (argc >= 5 && counterfactual_policy_command) {
        return run_counterfactual_policy_preference_check(
            ...
            argv[3], argv[4], policy_preference_output_path, 0u,
            (size_t)policy_preference_epochs,
            (size_t)policy_preference_minibatch_pairs,
            (size_t)policy_preference_early_stop_patience,
            learning_rate_override > 0.0f ? learning_rate_override : 0.00001f,
    ...
    if (argc >= 4 && strcmp(argv[1], "--train-live-awr") == 0) {
        return train_from_input_file(
            argv[2],
            argv[3],
            TRAIN_INPUT_EPISODE_BATCH,
            TRAIN_METHOD_ADVANTAGE_WEIGHTED_IMITATION,
            epochs,
            learning_rate_override,
            rl_gamma,
            rl_entropy_coef,
    ...
    return rc;
        ...
        "  showdown_client --train-live-awr <episode_batch.jsonl> <checkpoint.bin> [--epochs N] [--learning-rate F] [--gamma F] [--entropy-coef F] [--advantage-norm 0|1] [--awr-minibatch-episodes N] [--awr-temperature F] [--awr-max-weight F] [--target-kl F] [--shuffle-seed N] [--episode-limit N] [--reward-mode terminal|dense_additive] [--policy-tag-expected TAG]\n"
