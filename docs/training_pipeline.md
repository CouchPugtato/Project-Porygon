...
Search evaluations also use matched randomness. Every candidate, training replicate, and player-side swap in a given stage/block receives the same deterministic battle and random-team seed sequence. This makes candidate differences less sensitive to lucky team draws while retaining balanced A/B evaluation. The self-play launcher installs an opt-in hook in the ignored local Showdown checkout; ordinary self-play remains unseeded unless `--battle-seed-base` is supplied. Evaluation summaries record the seed and resumed searches reject older unmatched artifacts.
