...
The default search is a conservative 256-episode sweep over learning rates `2.5e-6` through `1e-5`, anchor coefficients `0.01` through `0.05`, and PPO clips `0.1` and `0.2`. It uses shuffle seeds `101,202,303`, equal screening allocations, and pooled parameter-setting ranking. `max_trials` counts checkpoints, so each three-seed setting consumes three trials; `finalists` counts parameter settings in replicated mode. The manifest under `models/search/<run-prefix>/` contains seed-level diagnostics, pooled group rankings, confidence intervals, `best_hyperparameters`, a representative `top_result`, and a separate promotion assessment. `tentative_winner` means the pooled point estimate passed but its confidence interval did not clear the parent; only `confident_winner` supports automatic promotion. A search result is evidence for the tested parent, batch, opponent, and parameter ranges—not a universally optimal PPO setting.
